...
export class MeController {
  ...
  public async getEventById(@ApiContext() context: Context, @Param("eventId") eventId: string): Promise<UserEvent> {
    ...
    return {
      company: {
        name: company.name,
        logo: company.information.logo.toString()
      },
      event,
      chairmanIdentity,
      shareholderIdentity,
  ...
  public async askQuestion(context: Context, eventId: string, payload: AskQuestionDto) {
    ...
    const companyId = eventRoles[0].companyId as string;
    const result = await this.auditRepo.logQuestion({
      _id: null,
      eventId,
      userId: context.id,
      data: QuestionAuditData.create(name, eventRoles as any, payload.question)
    });
